...
t=(0:tau:(N-1)*tau);
t =t.';
S1 = -4.*(abs(x)<0.1 & 0.1 <t & t<0.6);

%S1 = @(t,x) -4*(x<0.1 && t>0.1  && t<0.6);

x = -L-h/2:h:L+h/2;
D = 1 -(abs(x)./10).*(abs(x)<=L); 

for t1 = 1:N-1
    for x1 = 2:M-1
        u1(t1+1,x1) = u1(t1,x1) + (tau/h^2)*D(x1+1)*(u1(t1,x1+1)-u1(t1,x1)) + (tau/h^2)*D(x1-1)*(u1(t1,x1-1)-u1(t1,x1)) + tau*S1(t1,x1);
    end
end
